import { Injectable } from '@angular/core';
import {Cart} from '../../interfaces'
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';
// export interface Cart{
//   Product_id: number
//   Product_name:string
//   Product_description:string
//   Product_price:number
//   Count:number
//   Product_image:string
//   User_id:string,
//   Order_id:string
// }

@Injectable({
  providedIn: 'root'
})
export class CartService {

  cart:Cart[]=[]
    // {  
    //   Product_id: 1,
    //   Product_name:'Food',
    //   Product_description:'delicious',
    //   Product_price: 200,
    //   Count:1,
    //   Product_image:"data:image/png;base64,",
    //   User_id:'15',
    //   Order_id:'3'

    // },

    // { 
    //    Product_id: 2,
    //   Product_name:'Clothes',
    //   Product_description:'all sizes',
    //   Product_price: 1500,
    //   Count:1,
    //   Product_image:"data:image/png;base64,",
    //   User_id:'15',
    //   Order_id:'3'
    // },
    // {  
    //   Product_id: 3,
    //   Product_name:'Furniture',
    //   Product_description:'strong and durable',
    //   Product_price: 120000,
    //   Count:1,
    //   Product_image:"data:image/png;base64,",
    //   User_id:'21',
    //   Order_id:'5',
    // }
    // ]
 

  constructor(private http:HttpClient) { }

  getoneProduct(id:string){
    let pos =this.cart.findIndex(product=>product.Product_id===id)
    return this.cart[pos]
  }

  getAllProducts():Observable<Cart[]>{
    return this.http.get<Cart[]>('http://localhost:4000/Cart/af3794b4-8884-493e-841e-f138cf6325a7')
  }

  addProduct(newProduct:Cart){
    this. cart.push(newProduct)
  }

  increaseItemInCart(id:string){
    let incItem = this.cart.find(product=>{return product.Product_id===id})
    if(incItem){
      incItem.count= incItem.count+1
    }
  }

  decreaseItemInCart(id:string){
    let decItem = this.cart.find(product=>{return product.Product_id===id})
    if(decItem){
      if (decItem.count>1){
      decItem.count = decItem.count-1
    }
    else
    {
      this.cart.splice(this.cart.indexOf(decItem),1)
    }
  }
  }

  deleteFromCart(id:string){
    let deleteItem = this.cart.find(product=>{return product.Product_id===id})
    if(deleteItem){
      this.cart.splice(this.cart.indexOf(deleteItem),1)
    }
  }

}
